...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    'distilbert-base-uncased' : "C:/Users/14804/models/distilbert-base-uncased.tar.gz",
...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            dim: Size of the encoder layers and the pooler layer.
            n_layers: Number of hidden layers in the Transformer encoder.
            n_heads: Number of attention heads for each attention layer in
                the Transformer encoder.
            hidden_dim: The size of the "intermediate" (i.e., feed-forward)
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = BertLayerNorm(config.dim, eps=1e-12)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.dim, config.hidden_dim)
        self.intermediate_act_fn = ACT2FN[config.activation] \
            if isinstance(config.activation, str) else config.activation
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.hidden_dim, config.dim)
        self.LayerNorm = BertLayerNorm(config.dim, eps=1e-12)
        self.dropout = nn.Dropout(config.dropout)
...
class BertOnlyNSPHead(nn.Module):
    def __init__(self, config):
        ...
        self.seq_relationship = nn.Linear(config.dim, 2)
...
class BertModel(PreTrainedBertModel):
    ...
    config = modeling.BertConfig(vocab_size_or_config_json_file=32000, dim=768,
        n_layers=12, n_heads=12, hidden_dim=3072)
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    config = BertConfig(vocab_size_or_config_json_file=32000, dim=768,
        n_layers=12, n_heads=12, hidden_dim=3072)
...
class BertMultiwayMatch(PreTrainedBertModel):
    ...
    def __init__(self, config, num_choices=4):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.dropout)
        self.linear_trans = nn.Linear(config.dim, config.dim)
        self.linear_fuse_p = nn.Linear(config.dim*2, config.dim)
        self.linear_fuse_q = nn.Linear(config.dim*2, config.dim)
        self.linear_fuse_a = nn.Linear(config.dim * 2, config.dim)
        self.classifier = nn.Linear(config.dim*3, 1)
        self.apply(self.init_bert_weights)
